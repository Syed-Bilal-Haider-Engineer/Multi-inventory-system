...
const ProductDetails = ({ data }) => {
  ...
  const toggleWishlistHandler = () => setClick(!click);
  ...
  const totalRatings = data?.reviews?.reduce((sum, review) => sum + review.rating, 0) || 0;
  const averageRating = (totalRatings / totalReviewsLength).toFixed(2);
...
const ProductDetailsInfo = ({ data, totalReviewsLength, averageRating }) => {
  ...
  return (
    <div className="bg-[#f5f6fb] px-3 800px:px-10 py-2 rounded">
      ...
      {active === 1 && (
        <p className="text-black py-2 text-[18px] leading-8 pb-10 whitespace-pre-line">
          {data.description}
        </p>
      )}
        ...
        <div className="w-full min-h-[40vh] flex flex-col items-center py-3 overflow-y-auto">
          {data?.reviews?.length > 0 ? (
              ...
              <div key={index} className="w-full flex my-2">
                ...
                <div className="pl-2">
                  <div className="w-full flex items-center">
                    <h1 className="font-[500] mr-3 text-black">{item.user.name}</h1>
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <i key={i}>
                          {i < item.rating ? (
                            <AiFillStar color="#f6b100" className="mr-2 cursor-pointer" size={20} />
                          ) : (
            ...
            <div className="w-full flex justify-center">
              <h5 className="text-black">No Reviews have for this product!</h5>
            </div>
          )}
        </div>
      )}
          ...
          <div className="w-full 800px:w-[50%] mt-5 800px:mt-0 800px:flex flex-col items-end">
            <div className="text-left">
              ...
              <h5 className="font-[600] pt-3 text-black">
                Total Products: <span className="font-[500]">{data.shop.totalProducts}</span>
              </h5>
              <h5 className="font-[600] pt-3 text-black">
